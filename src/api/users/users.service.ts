...
export class UsersService {
	...
	public static getInstance(db: DrizzleD1Database<Record<string, never>>, jwtSecret: string) {
		if (!UsersService.instance) UsersService.instance = new UsersService(db, jwtSecret);
	...
	public async signUp(data: { email: string; password: string }) {
		...
		const user = await this.db.insert(users).values({ email, password: hashedPassword! }).returning({ id: users.id }).get();
